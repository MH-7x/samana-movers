...
const ServiceRighSidebar = ({ service }: { service: string }) => {
  return (
    <div className="border  col-span-1">
      <h3 className="text-3xl px-10 pt-10 font-black b1">Our Service Areas</h3>
      ...
      <QuoteForm side />
      <h3 className="text-3xl px-10 pt-5 font-black b1">Our More Services</h3>
      <ul className="mt-5 b-text px-10 pb-10">
        {services.map((service, i) => (
          <Link key={i} href={service.href} title={service.title}>
            {" "}
            <li className="flex items-center gap-x-2 mb-3 hover:underline hover:underline-offset-1">
              <ArrowRightIcon className="w-4 h-4 b2" />
              {service.title}
...
export default ServiceRighSidebar;
